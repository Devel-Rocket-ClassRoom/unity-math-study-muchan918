...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < units.Length; i++)
        {
            ...
            // TODO
            //Quaternion slotRotation = leader.rotation * Quaternion.Euler(0f, angle * i, 0f);
            Quaternion slotRotation = Quaternion.AngleAxis(angle * i, leader.up);

            // TODO
            units[i].position = leader.position + slotRotation * Vector3.forward * formationRadius;
            units[i].rotation = slotRotation;
        }
